...
def web_page():
    """Generate the web page with current sky brightness."""

    # Zero magnitude constant and glass attenuation factor.  These values should
    # be calibrated against a reference device (e.g. SQM-LE/LU).
    M0 = -16.07
    GA = 5

    # Field of view of the 8 mm lens and corresponding solid angle.
    FOV_DEG = 41.1  # approximate FOV in degrees
    OMEGA_SR = 2 * math.pi * (1 - math.cos(math.radians(FOV_DEG) / 2))
    ...
    delta = full_C - ir_C
    if delta > 0:
        radiance = delta / OMEGA_SR
        mpsas = M0 + GA - 2.5 * math.log10(radiance)
    else:
        mpsas = 25.0  # very dark or other handling

    datal = f"{mpsas:.2f}"
    unit = "M/arcsec2"

    html = f"""<html><head> <title>SQM Web Server</title>
